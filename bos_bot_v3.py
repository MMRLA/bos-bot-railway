...
def normalize_spot_timestamp(raw_ts_ms: Optional[int]) -> int:
    ...
    Si viene invalido, devuelve hora UTC actual.
    NO incrementa contadores; eso se gestiona fuera.
    """
    now_ts = now_utc_ts()
...
def extract_spot_timestamp(spot):
    """
    Intenta extraer un timestamp usable del spot.
    Devuelve:
      - ts_sec
      - used_fallback (True/False)
      - source_field (str)
    """
    for attr in ("timestamp", "utcTimestamp", "eventTimestamp", "time"):
        if hasattr(spot, attr):
            raw = getattr(spot, attr)
            try:
                raw = int(raw)
                if raw > 0:
                    return normalize_spot_timestamp(raw), False, attr
            except Exception:
                pass

    return now_utc_ts(), True, "local_utc_fallback"
...
class BosBot:
        ...
        try:
            ...
            if state.pending_broker_h1_fetch:
                ...
                if not closed_candidates:
                    log.warning(
                        f"No hay barras broker cerradas validas para comparar | "
                        f"current_hour_start={fmt_dt_utc(current_hour_start)}"
                    )
                    return
                ...
                if state.last_local_closed_bar is None:
                    log.warning(
                        "H1 broker recibida pero no existe last_local_closed_bar para comparar."
                    )
                    return
    ...
    def _on_spot(self, message):
        ...
        ts, used_fallback, ts_source = extract_spot_timestamp(spot)
    ...
    def _on_reconcile(self, message):
        ...
        for p in server_positions:
            try:
                ...
                server_open_ids.add(pos_id)

                # ---------------------------------------------------------
                # Sincronizar margen de trades ya abiertos en estado local
                # usando el usedMargin real del broker
                # ---------------------------------------------------------
                ...
                if not math.isnan(margin_used_raw) and margin_used_raw > 0:
                    broker_margin = margin_used_raw / (10 ** money_digits)

                    if pos_id in state.open_trades:
                        local_margin = state.open_trades[pos_id].margin_used

                        if math.isnan(local_margin) or abs(local_margin - broker_margin) > 0.01:
                            state.open_trades[pos_id].margin_used = broker_margin
                            log.info(
                                f"RECONCILE_MARGIN_SYNC | positionId={pos_id} | "
                                f"local_margin={local_margin:.2f} -> broker_margin={broker_margin:.2f}"
                            )
                ...
                log.info(
                    ...
                    f"usedMargin={getattr(p, 'usedMargin', None)} | "
                    f"moneyDigits={getattr(p, 'moneyDigits', None)} | "
                    f"raw={p}"
                )
            except Exception as e:
                log.warning(f"Error procesando posicion en reconcile: {e}")
